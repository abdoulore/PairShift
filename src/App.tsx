...
import { ArrowsLeftRight, ChatText, Lightning, Pulse, ShieldCheck, Wallet } from "@phosphor-icons/react";
import { ASSETS, ASSET_BY_TICKER, tokenSymbol } from "../shared/assets";
import { fmtUsd } from "../shared/math";
import { cancelMessage, intentMessage } from "../shared/messages";
import { DEFAULT_LIMITS, PRE_IPO_SLIPPAGE_BPS, canonicalDraft, type Intent, type IntentDraft, type MarketSnapshot, type Mode, type Status } from "../shared/types";
...
export function App() {
  ...
  async function arm() {
    ...
    try {
      ...
      if (mode === "paper") {
        ...
        say("Paper switch armed. PairShift is watching Pyth prices.");
        return;
      }
  ...
  async function confirmSwitch(i: Intent) {
    setBusy(i.id);
    try {
  ...
  // Tell the user when a one-tap switch becomes ready.
  ...
  useEffect(() => {
    ...
    for (const i of ready) {
      ...
      const msg = `Ready to switch ${i.from} into ${i.to}. Confirm in PairShift.`;
      say(msg);
      try {
  ...
  async function cancel(i: Intent) {
    setBusy(i.id);
    try {
      if (i.mode === "paper") {
  ...
  const refLabel = pairSources.has("fallback")
    ? "fallback reference prices"
    : pairSources.has("prestocks") && pairSources.has("pyth")
      ? "Pyth and PreStocks reference prices"
      : pairSources.has("prestocks")
        ? "PreStocks mark prices"
        : "Pyth reference prices";
  ...
  return (
    <>
      <header className="topbar">
        ...
        {status && (
          <span className={`chip hide-sm ${status.source === "pyth" ? "good" : "warn"}`} title={status.sourceNote}>
            <Pulse size={14} weight="bold" />
            {status.source === "pyth" ? "Pyth live" : "Fallback prices"}
          </span>
        )}
        {status && (
          <span
            className={`chip hide-sm ${status.liveEnabled ? (status.autoEnabled ? "good" : "warn") : ""}`}
      ...
      <main className="page">
        <div className="intro">
          ...
          <p>
            Tell PairShift when one tokenized stock, public or pre-IPO, has become cheap enough relative to another. It checks Pyth and PreStocks prices and
            swaps atomically on Solana.
          </p>
        </div>
          ...
          <div className="stack">
            <section className="card composer">
              <div className="card-pad">
                ...
                <div className="sentence">
                  ...
                  when <strong>{draft.to}</strong>{" "}
                  {draft.direction === "cheaper" ? "becomes" : "outperforms"}{" "}
                ...
                {preview?.thinTrigger !== undefined && (
                  <p className="hint warn" style={{ marginTop: 8 }}>
                    Fees and spread on this route are about {preview.thinTrigger.toFixed(1)}%, which would eat most of a {preview.draft.thresholdPct}% move.
                    Consider a trigger of {Math.ceil(preview.thinTrigger * 3)}% or more.
                  </p>
                )}
              </div>
              ...
              <div className="plan">
                <div>
                  ...
                  <div className="s">{preview ? `${preview.draft.from} shares, from ${refLabel}` : " "}</div>
                </div>
                <div>
            ...
            <section className="card">
              <div className="card-head">
                ...
                <span className="sub">
                  From {refLabel}. Shaded area is where the switch fires.
                </span>
              </div>
              <PairChart
                series={series}
          ...
          <aside className="card">
            ...
            <div className="side-actions">
              ...
              {mode === "paper" ? (
                ...
                style === "confirm" ? (
                  <p className="hint">
                    {draft.from} charges a 1% transfer fee, so PairShift won't move it an extra time. When the trigger fires and every check passes, you confirm the swap
                    in one tap.
                  </p>
                ) : (
              ...
              <button className="btn btn-primary" onClick={arm} disabled={!preview || sameAsset || busy === "arm" || (draft.mode === "live" && !liveReady)}>
                {mode === "live" ? <Wallet size={18} weight="bold" /> : <ShieldCheck size={18} weight="bold" />}
                {busy === "arm" ? "Arming" : mode === "live" ? (style === "confirm" ? "Sign and arm" : "Approve and arm") : "Arm paper switch"}
              </button>
        ...
        <div className="section-title">
          ...
          <span className="muted" style={{ fontSize: 12.5 }}>
            {wallet.connected ? "This wallet and this browser" : "This browser (paper)"}
          </span>
        </div>
        <IntentList intents={intents} onCancel={cancel} onConfirm={confirmSwitch} busyId={busy ?? undefined} />
        ...
        <div className="how">
          <div>
            <ChatText size={22} className="ico" />
            <h3>Say it plainly</h3>
            <p>PairShift turns your sentence into a ratio trigger, measured from today's Pyth prices.</p>
          </div>
          <div>
            <Wallet size={22} className="ico" />
            <h3>Keep custody</h3>
            <p>You approve an exact amount to the keeper. Nothing leaves your wallet before the switch.</p>
          </div>
          <div>
            <ShieldCheck size={22} className="ico" />
            <h3>Verified before it trades</h3>
            <p>Fresh and tight Pyth prices, tokens near their stock, no dividend or split in flight.</p>
          </div>
          <div>
            <ArrowsLeftRight size={22} className="ico" />
            <h3>One atomic swap</h3>
            <p>Pull, swap through Jupiter, deliver the new stock. One transaction with a hard minimum out.</p>
          </div>
        </div>
      </main>
